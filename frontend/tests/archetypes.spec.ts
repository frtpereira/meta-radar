...
test("archetypes page paginates and filters archetypes in the browser", async ({
    page,
}) => {
...
test("archetypes page shows empty and server-error fallback states", async ({
    page,
}) => {
    await page.goto("/archetypes?meta_id=meta-empty");
    await expect(page.getByText("No archetypes found")).toBeVisible();

    await page.goto("/archetypes?meta_id=meta-error");
    await expect(page.getByText("No archetypes found")).toBeVisible();
});

test("decklist detail renders the card list and matchup summaries", async ({
    page,
}) => {
    await page.goto("/archetypes/1?meta_id=meta-2026");

    await expect(
        page.getByRole("heading", { name: "Charizard ex" }),
    ).toBeVisible();
    await expect(
        page.getByRole("heading", { name: "Core/Skeleton" }),
    ).toBeVisible();
    await expect(page.getByText("Charmander")).toBeVisible();
    await expect(page.getByText("Rare Candy")).toBeVisible();
    await expect(
        page.getByRole("heading", { name: "Optional Cards" }),
    ).toBeVisible();
    await expect(page.getByText("Buddy-Buddy Poffin")).toBeVisible();
    await expect(
        page.getByRole("heading", { name: "Best Against" }),
    ).toBeVisible();
    await expect(
        page.getByRole("link", { name: "View all matchups →" }),
    ).toBeVisible();
});
