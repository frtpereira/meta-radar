...
const archetypes = [
    ...Array.from({ length: 24 }, (_, index) =>
        makeArchetype(index + 1, `Archetype ${String(index + 1).padStart(2, "0")}`),
    ),
...
describe("ArchetypeSearch", () => {
    it("renders the first page of results and paginates locally", async () => {
        ...
        render(React.createElement(ArchetypeSearch, { archetypes, metaId: "meta-1" }));
    ...
    it("links each archetype row to the matching detail page with the active meta id", () => {
        ...
        expect(screen.getByRole("link", { name: "Archetype 01" })).toHaveAttribute(
            "href",
