...
// Offset from the cursor, and the preview's rendered width -- kept in
// sync with the `.card-preview-image` width in globals.css so the
// right-edge overflow check below is accurate.
const OFFSET_X = 20;
const OFFSET_Y = 20;
...
export default function CardHoverPreview({
    ...
    const updateCoords = (e: React.MouseEvent) => {
        ...
        frame.current = requestAnimationFrame(() => {
            setCoords({ top: e.clientY + OFFSET_Y, left });
            frame.current = null;
        });
    };
