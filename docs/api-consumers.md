...
| `GET /api/metas` | `getMetas` | `app/page.tsx`, `app/tournaments/page.tsx`, `app/matchups/page.tsx`, `app/archetypes/page.tsx` — all four use it to populate the meta selector dropdown (grouped by standard meta via `groupMetasForSelect`) and, via `lib/metas.ts`'s `pickDefaultMeta`, to pick which meta the page defaults to (the requested `meta_id` if present, else the currently open standard meta, else the newest meta) |
| `GET /api/metas/current` | `getCurrentMetas` | *None.* Defined in the client but not called from any page — the four pages above derive the default meta client-side from `/api/metas`'s full list instead (see `pickDefaultMeta`), since they need that full list for the selector dropdown anyway. Exists as a lighter-weight lookup for a future consumer that only needs "what's current," not the full history. |
